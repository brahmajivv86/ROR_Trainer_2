...
function startSession() {
    ...
    if (selectedCategory === 'iala') {
        const ialaCardIds = Object.keys(cardsData.iala || {}).sort((a, b) => parseInt(a) - parseInt(b));
        const idsToUse = ialaCardIds.length > 0 ? ialaCardIds : Array.from({length: 40}, (_, i) => (i + 1).toString());

        idsToUse.forEach(id => {
            currentQueue.push({ cardId: id, type: 'iala' });
        });
        ...
        const flagCardIds = Object.keys(cardsData.flags || {}).sort((a, b) => parseInt(a) - parseInt(b));
        const idsToUse = flagCardIds.length > 0 ? flagCardIds : Array.from({length: 40}, (_, i) => (i + 1).toString());

        idsToUse.forEach(id => {
            currentQueue.push({ cardId: id, type: 'flags' });
        });
        ...
        if (sessionMode === 'trainer') {
            sessionModeBadge.textContent = 'Flags Trainer';
            sessionModeBadge.className = 'badge badge-teal';
            showView('trainer');
            loadTrainerCard();
        } else {
            sessionModeBadge.textContent = 'Flags Quiz';
            sessionModeBadge.className = 'badge';
            showView('quiz');
            loadQuizQuestion();
        }
...
function loadTrainerCard() {
    ...
    // Toggle back wrappers: text panel active
    trainerBackImageWrapper.classList.add('hidden');
    trainerBackScroll.classList.remove('hidden');

    if (type === 'iala') {
        trainerImg.src = `images/iala/bouysimage${item.cardId}.gif`;
        trainerImg.alt = `IALA Buoyage #${item.cardId}`;
        trainerCardLabel.textContent = `IALA Buoyage #${item.cardId}`;
        trainerBackTitle.textContent = card.title || `IALA Buoyage Signal #${item.cardId}`;
        trainerInfoIdent.textContent = card.identification || 'No description available.';
        ...
        trainerInfoIdent.textContent = card.identification || 'No meaning specified.';
        trainerInfoAction.textContent = card.action || '-';
        trainerInfoDay.textContent = card.day_signal || '-';
        trainerInfoFog.textContent = card.fog_signal || '-';
    } else if (type === 'day') {
        ...
        trainerBackTitle.textContent = `Signal Reference Details (Day Card #${item.cardId})`;
        trainerInfoIdent.textContent = card.identification || 'No identification text.';
        trainerInfoAction.textContent = card.action || 'No action specified.';
        trainerInfoDay.textContent = card.lights_displayed || 'No lights specified.';
